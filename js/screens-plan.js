...
(function () {
  ...
  function Budget() {
    ...
    const row = (b) => {
      ...
      return html`<button key=${b.cat} class="lrow" style=${{ flexDirection: 'column', alignItems: 'stretch', gap: '8px' }} onClick=${() => openSheet({ k: 'budgetEdit', cat: b.cat })}>
        <span class="between" style=${{ gap: '8px' }}><span class="row" style=${{ gap: '10px', minWidth: 0, flexWrap: 'wrap' }}><${Tile} icon=${c.icon} tone=${c.tone} s=${30} /><span class="t1" style=${{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>${c.name}</span>${over && html`<span class="pill warn" style=${{ height: '22px' }}><${Icon} n="alert" s=${11} w=${2.4} />Over</span>`}</span><span class="small num" style=${{ flexShrink: 0, textAlign: 'right' }}><b>${fmt(b.actual)}</b><span class="muted"> of ${fmt(b.plan)}</span></span></span>
  ...
  function Bills() {
    ...
    return html`<div class="stack">
      <div class="card stack" style=${{ gap: '12px' }}><div class="between"><span class="stack-s" style=${{ gap: '2px' }}><span class="small muted">Monthly commitments</span><span class="disp num" style=${{ fontSize: '26px', fontWeight: 800 }}>${fmt(total)}</span></span><span class="small muted num">${fmt(total * 12)} a year</span></div>
        <${Stripe} parts=${parts.map(([k, c]) => [tot(k), c])} h=${12} /><div class=${wide ? 'grid w4' : 'grid'} style=${wide ? { gap: '8px 16px' } : { gap: '8px 16px', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))' }}>${parts.map(([k, c]) => html`<${KeyRow} key=${k} color=${c} label=${k} value=${fmt(tot(k))} />`)}</div></div>
  ...
  K.GoalDetail = function GoalDetail({ route }) {
    ...
    const left = html`<div class="stack">
      ...
      <${DangerButton} label="Delete goal" ask="Delete this goal?" onConfirm=${() => { commit(K.remove(data, 'goals', g.id)); toast('Goal deleted'); back(); }} /></div>`;
  ...
  K.TripDetail = function TripDetail({ route }) {
    ...
    const left = html`<div class="stack">
      ...
      ${t.txns.length > 0 && html`<${K.ChartBox} title="Daily spending" question="Which days cost the most?"><${BarChart} groups=${perDay.map((v, i) => ({ label: String(dayList[i].getDate()), values: [v] }))} colors=${['var(--info)']} h=${150} hi=${perDay.indexOf(Math.max(...perDay))} labelTop=${(v) => fmt.k(v)} fmtY=${fmt.k} names=${['Spent']} fmtV=${(v) => fmt(v)} /></${K.ChartBox}>`}
    ...
    const right = html`<div class="stack">
      ...
      <${DangerButton} label="Delete trip" ask="Delete this trip?" onConfirm=${() => { commit(Object.assign({}, K.remove(data, 'trips', t.id), { txns: data.txns.map((x) => (x.trip === t.id ? Object.assign({}, x, { trip: null }) : x)) })); toast('Trip deleted · its expenses were kept'); back(); }} /></div>`;
