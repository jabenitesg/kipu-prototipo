...
(function () {
  ...
  const TxnRow = ({ t }) => {
    ...
    const src = t.split && t.split.by === 'partner' ? 'Paid by ' + who : K.whereName(data, t.from || t.to);
  ...
  K.RecurringNote = function RecurringNote() {
    ...
    const loanish = (r) => K.LOAN_WORDS.test(r.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
    if (!list.length) return null;
    return html`<div class="card stack-s" style=${{ gap: '8px', borderColor: 'color-mix(in srgb, var(--acc) 35%, var(--line))' }}><span style=${{ fontWeight: 700 }}>Payments that repeat</span><span class="small muted" style=${{ lineHeight: 1.45 }}>Kipu found these every month in your statements. Are they bills? A loan taken automatically goes in Loans instead.</span>
      <div class="list">${list.map((r) => html`<div key=${r.where + r.key} class="lrow" style=${{ gap: '10px', flexWrap: 'wrap' }}><span class="grow stack-s" style=${{ gap: '1px', minWidth: '160px' }}><span class="t1" style=${{ fontSize: '14px' }}>${r.name}</span><span class="t2">${fmt.native(r.amt, (K.whereItem(data, r.where) || {}).cur || data.base, { dec: 2 })} · ${K.ord(r.day) + ' of the month'} · ${K.whereName(data, r.where)}</span></span><div class="grid rec-actions" style=${{ gap: '6px', flexBasis: '100%', gridTemplateColumns: '1fr 1fr' }}><button class="btn sec sm" style=${loanish(r) ? null : { gridColumn: '1' }} onClick=${() => { commit(K.dismissRecurring(data, [r])); toast(r.name + ' won’t be suggested again'); }}>Not a bill</button>${loanish(r) && html`<button class="btn sec sm" onClick=${() => openSheet({ k: 'addLoan', preset: { fromRecurring: r } })}>It’s a loan</button>`}<button class="btn pri sm" style=${loanish(r) ? { gridColumn: '1 / -1' } : null} onClick=${() => { commit(K.addRecurringBills(data, [r])); toast(r.name + ' added as a bill'); }}>Add bill</button></div></div>`)}</div></div>`;
  };
  ...
  // Bills whose price changed: said once when it updated itself, asked when the jump is big
  K.PriceNote = function PriceNote() {
    const { data, commit, fmt } = useApp();
    const cur = (b) => b.cur || data.base;
    const changed = data.bills.filter((b) => b.lastChange && !b.lastChange.seen && K.days(K.parse(b.lastChange.date), K.today()) <= 90);
    const pending = data.bills.filter((b) => b.pendingPrice);
    if (!changed.length && !pending.length) return null;
    return html`<div class="card tight list">
      ${changed.map((b) => html`<div key=${'c' + b.id} class="lrow" style=${{ gap: '10px', flexWrap: 'wrap' }}><span class=${'ic ' + (b.lastChange.to < b.lastChange.from ? 'g' : 'a')} style=${{ width: '30px', height: '30px', borderRadius: '10px' }}><${Icon} n=${b.lastChange.to < b.lastChange.from ? 'down' : 'up'} s=${15} w=${2.2} /></span><span class="grow stack-s" style=${{ gap: '1px', minWidth: '150px' }}><span class="t1" style=${{ fontSize: '14px' }}>${b.name + (b.lastChange.to < b.lastChange.from ? ' went down' : ' went up')}</span><span class="t2 num">${fmt.native(b.lastChange.from, cur(b), { dec: 2 })} → ${fmt.native(b.lastChange.to, cur(b), { dec: 2 })}</span></span><button class="btn sec sm" onClick=${() => commit(K.seenPrice(data, b.id))}>OK</button></div>`)}
      ${pending.map((b) => html`<div key=${'p' + b.id} class="lrow" style=${{ gap: '10px', flexWrap: 'wrap' }}><span class="ic a" style=${{ width: '30px', height: '30px', borderRadius: '10px' }}><${Icon} n="alert" s=${15} w=${2.2} /></span><span class="grow stack-s" style=${{ gap: '1px', minWidth: '150px' }}><span class="t1" style=${{ fontSize: '14px' }}>${b.name + ': new price?'}</span><span class="t2 num">${'Last payment ' + fmt.native(b.pendingPrice.amt, cur(b), { dec: 2 }) + ' · bill ' + fmt.native(b.amt, cur(b), { dec: 2 })}</span></span><div class="row" style=${{ gap: '6px' }}><button class="btn sec sm" onClick=${() => commit(K.keepPrice(data, b.id))}>Keep</button><button class="btn pri sm" onClick=${() => commit(K.acceptPrice(data, b.id))}>Update</button></div></div>`)}</div>`;
  };
  // Loan payments imported as spending before Kipu knew the loan
  K.LoanPayNote = function LoanPayNote() {
    const { data, commit, toast, fmt } = useApp();
    const list = K.misfiledLoanPayments(data);
    if (!list.length) return null;
    const total = list.reduce((a, t) => a + (t.base || 0), 0);
