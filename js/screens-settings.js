...
(function () {
  ...
  function Appearance() {
    ...
    const [pv, setPv] = useState(settings.effectiveDark ? 'Dark' : 'Light');
    const T = K.THEMES[pick];
    return html`<${SubHead} title="Appearance & theme" sub="Brightness and color theme are set separately." />
      <div class="stack-s"><span class="eyebrow">Appearance</span><${Seg} options=${['Light', 'Dark', 'System']} value=${settings.mode} onChange=${(m) => setSettings({ mode: m })} /></div>
      ...
      <div class="card flat stack" style=${{ gap: '14px' }}><div class="between"><span class="stack-s" style=${{ gap: '2px' }}><span style=${{ fontWeight: 700 }}>${pick}</span><span class="tiny muted">${T.desc}</span></span>${!wide && html`<div style=${{ width: '140px' }}><${Seg} options=${['Light', 'Dark']} value=${pv} onChange=${setPv} /></div>`}</div>
        <div class="row" style=${{ justifyContent: 'center', gap: '16px', flexWrap: 'wrap', padding: '16px', borderRadius: '18px', background: 'var(--surface)' }}>${wide ? html`<${ThemePreview} name=${pick} dark=${false} w=${200} /><${ThemePreview} name=${pick} dark=${true} w=${200} />` : html`<${ThemePreview} name=${pick} dark=${pv === 'Dark'} />`}</div>
