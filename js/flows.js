...
(function () {
  ...
  const confirmSpaceEdit = (data, coll, item, shared) => {
    if (!item || !!item.shared === !!shared) return true;
    const move = K.moveSpace(data, coll, item.id);
    return window.confirm('This change also moves ' + move.items.length + ' linked item(s) and ' + move.transactions + ' transaction(s) to ' + (shared ? 'the Household' : 'your private space') + '.\n\nLinked items: ' + move.items.join(', ') + '\n\nContinue?');
  };
  ...
  const StatementSheet = ({ onClose }) => {
    ...
    const doImport = () => {
      ...
      d = Object.assign({}, d, { imports: [{ id: imp, name: st.name, when: K.iso(K.today()), count: chosen.length, where, from: chosen.map((r) => r.date).filter(Boolean).sort()[0], to: chosen.map((r) => r.date).filter(Boolean).sort().pop(), settled: paidCount, kind: st.kind && st.kind.card ? 'card' : undefined, shared: impShared }].concat(d.imports) });
  ...
  const AddIncomeSource = ({ onClose, item }) => {
    ...
    const save = () => { if (!confirmSpaceEdit(data, 'income', item, K.isShared(data, f.shared))) return; const next = K.upsert(data, 'income', Object.assign({}, item || {}, { name: f.name || 'Income', amt: numv(f.amt), cur: f.cur, freq: f.freq, next: f.next, to: f.to, shared: K.isShared(data, f.shared) })); commit(next); const p = K.derive(next, ctx).plan; toast('Income saved · Safe to Spend ' + fmt(p.safe)); onClose(); };
