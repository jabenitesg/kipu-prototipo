...
(function () {
  ...
  const longDate = () => new Date().toLocaleDateString(K.loc(), { weekday: 'long', month: 'long', day: 'numeric' });
  ...
  const SplitCard = () => {
    const { data, fmt, openSheet } = useApp();
    if (K.hhMode(data) !== 'mixed') return null;
    const who = K.partnerName(data) || 'Partner';
    const { owed, items } = K.splitBalance(data);
    const text = !items.length ? 'Mark an expense as shared and Kipu keeps track of who owes whom.' : Math.abs(owed) < 0.01 ? 'You’re even.' : owed > 0 ? who + ' owes you ' + fmt(owed, { dec: 2 }) : 'You owe ' + who + ' ' + fmt(-owed, { dec: 2 });
  ...
  const InviteCard = () => {
    ...
    if (K.hhMode(data) !== 'together' || cloud.target === 'household' || data.household.inviteHidden) return null;
    const who = K.partnerName(data) || 'your partner';
    return html`<div class="card stack-s" style=${{ gap: '10px' }}><div class="row" style=${{ gap: '12px' }}><${Tile} icon="people" tone="p" /><span class="grow stack-s" style=${{ gap: '2px' }}><span style=${{ fontWeight: 700 }}>${'Bring ' + who + ' in'}</span><span class="small muted" style=${{ lineHeight: 1.45 }}>${'A joint Household shows both of you the same accounts, bills and spending, on each phone.'}</span></span></div>
  ...
  const QuickEntry = ({ compact, onDone }) => {
    ...
    // Your most frequent places lately: tap one, type the amount, done
    const recent = React.useMemo(() => {
      const since = K.iso(K.addDays(K.today(), -60)), n = {};
      data.txns.forEach((t) => { if (t.type === 'expense' && !t.recurring && t.merchant && t.date >= since && t.merchant.length <= 18) n[t.merchant] = (n[t.merchant] || 0) + 1; });
      return Object.keys(n).sort((a, b) => n[b] - n[a]).slice(0, 4);
    }, [data.txns]);
    if (!from) return null;
    const save = (e) => {
      e.preventDefault();
      ...
      const t = { type: 'expense', merchant: q.merchant || 'Expense', cat: K.guessCat(data, q.merchant) || 'other', amt: q.amt, cur, from, date: q.date, source: 'manual', shared: K.isShared(data) };
    ...
    const pick = (m) => { setText(m + ' '); if (inputRef.current) inputRef.current.focus(); };
    return html`<form class=${compact ? 'stack-s' : 'card stack-s'} style=${{ gap: '8px', padding: compact ? '4px 12px 8px' : '12px 14px' }} onSubmit=${save}>
      <div class="row" style=${{ gap: '8px' }}><input ref=${inputRef} class="input grow quick-in" aria-label="Quick expense" enterkeyhint="done" autocomplete="off" autocorrect="off" spellcheck="false" value=${text} onInput=${(e) => setText(e.target.value)} placeholder="45 Wong" /><button class="btn pri sm" disabled=${!text.trim()} aria-label="Add">${html`<${Icon} n="check" s=${18} w=${2.6} />`}</button></div>
      ${recent.length > 0 && !text && html`<div class="chips" style=${{ gap: '6px' }}>${recent.map((m) => html`<button key=${m} type="button" class="chip" onClick=${() => pick(m)}>${m}</button>`)}</div>`}
  ...
  K.Home = function Home() {
    ...
    const hello = first ? ', ' + first + (pal ? ' & ' + pal : '') : '';
    const top = insights[0];
    const trip = D.activeTrip;
    const month = html`<button class="kpis m" onClick=${() => go({ r: 'stats', tab: 'overview' })} aria-label="This month in Statistics">
      <span class="kpi"><span class="v">${fmt.ctx(D, m.income)}</span><span class="l">Income</span></span>
      <span class="kpi"><span class="v">${fmt.ctx(D, m.spending)}</span><span class="l">Spent${D.plan.budgetPlan ? ' of ' + fmt.k(D.plan.budgetPlan) : ''}</span></span>
      <span class="kpi"><span class="v">${fmt.ctx(D, m.saved)}</span><span class="l">Saved</span></span></button>`;
    ...
    const credit = D.cards.length > 0 && html`<button class="card solid" style=${{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }} onClick=${() => go({ r: 'money', tab: 'cards' })}>
      <span class="between"><span class="row" style=${{ gap: '10px' }}><span class="ic" style=${{ background: 'color-mix(in srgb, var(--hero-ink) 16%, transparent)', color: 'var(--hero-ink)' }}><${Icon} n="card" s=${17} /></span><span class="stack-s" style=${{ gap: '2px', textAlign: 'left' }}><span class="t1" style=${{ fontWeight: 600 }}>Credit utilization</span><span class="t2" style=${{ color: 'color-mix(in srgb, var(--hero-ink) 76%, transparent)' }}>${fmt(D.cardBal)} of ${fmt(D.cardLimit)}</span></span></span><span class="disp num" style=${{ fontWeight: 800, fontSize: '22px' }}>${D.util.toFixed(1)}%</span></span>
      <${K.Segs} pct=${D.util} color=${over ? '#F6C36A' : 'var(--hero-ink)'} track="color-mix(in srgb, var(--hero-ink) 22%, transparent)" mark=${data.prefs.utilRef} markColor="var(--hero-ink)" />
      <span class="tiny" style=${{ textAlign: 'left', fontWeight: 600, color: over ? '#FFD58A' : 'color-mix(in srgb, var(--hero-ink) 76%, transparent)' }}>${over ? 'Above your ' + data.prefs.utilRef + '% reference' : 'Under your ' + data.prefs.utilRef + '% reference'}</span></button>`;
    const tripStyled = trip && trip.look && trip.look.kind && trip.look.kind !== 'theme';
    const tripInk = trip ? K.inkFor(trip.look) : '#FFFFFF';
    const tripPct = trip && trip.budget > 0 ? (trip.spent / trip.budget) * 100 : 0;
    const tripCard = trip && html`<button class="card" style=${Object.assign({ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }, tripStyled ? { background: K.lookBg(trip.look), color: tripInk, border: 0, boxShadow: 'var(--glow)' } : {})} onClick=${() => go({ r: 'trip', id: trip.id })}>
      <span class="between"><span class="row" style=${{ gap: '10px' }}>${tripStyled ? html`<span class="ic" style=${{ background: 'color-mix(in srgb, ' + tripInk + ' 18%, transparent)', color: tripInk }}><${Icon} n="plane" s=${17} /></span>` : html`<${Tile} icon="plane" tone="p" />`}<span class="stack-s" style=${{ gap: '2px', textAlign: 'left' }}><span class="t1">${trip.name}</span><span class="t2" style=${tripStyled ? { color: 'color-mix(in srgb, ' + tripInk + ' 78%, transparent)' } : null}>Day ${trip.day} of ${trip.total}</span></span></span><span class="amt">${fmt(trip.spent)}</span></span>
      ${trip.budget > 0 && html`<${K.Segs} pct=${tripPct} color=${tripStyled ? tripInk : tripPct > 100 ? 'var(--crit2)' : tripPct > (trip.day / trip.total) * 100 + 10 ? 'var(--warn2)' : 'var(--acc)'} track=${tripStyled ? 'color-mix(in srgb, ' + tripInk + ' 25%, transparent)' : null} mark=${(trip.day / trip.total) * 100} />`}</button>`;
    ...
    const head = html`<header class="between" style=${{ paddingTop: wide ? 0 : '10px', alignItems: 'flex-start' }}><div class="stack-s" style=${{ gap: '4px' }}><span class="eyebrow">${longDate()}</span><h1 style=${{ fontSize: wide ? '30px' : '26px', fontWeight: 800 }}>${greeting()}${hello}</h1></div>${wide ? html`<${ContextFilter} show=${['scope', 'currency']} />` : html`<${Avatar} />`}</header>`;
    const ctxChip = !wide && (D.countries && D.countries.length > 1) && html`<div><${ContextFilter} show=${['scope', 'currency']} /></div>`;
    if (wide) {
      ...
      return html`<div class="stack" style=${{ gap: '26px' }}>
        ...
        ${D.noRate && D.noRate.length > 0 && html`<${K.RateNote} list=${D.noRate} />`}<${Setup} /><${DueSoon} /><${ProtectDevice} /><${HouseholdClosed} /><${SplitCard} /><${InviteCard} />
    ...
    return html`<div class="stack">${head}<${QuickEntry} />${ctxChip}${D.noRate && D.noRate.length > 0 && html`<${K.RateNote} list=${D.noRate} />`}<${Setup} /><${DueSoon} /><${SafeHero} /><${AskRow} /><${CountrySafe} /><${ProtectDevice} /><${HouseholdClosed} /><${SplitCard} /><${InviteCard} />${month}<${QuickRow} />${goalCard}${upcoming}${tripCard}${credit}${insight}</div>`;
  };
  ...
  function MoneyWide() {
    ...
    const Head = ({ title, total, sub, add, sheet }) => html`<div class="mw-head"><div class="stack-s" style=${{ gap: '2px', flex: 1, minWidth: 0 }}><h2>${title}</h2><span class="tiny muted mw-sub">${sub || ' '}</span></div><span class="row" style=${{ gap: '10px', flexShrink: 0 }}>${total != null && html`<b class="num">${total}</b>`}<button class="pill-link" onClick=${() => openSheet({ k: sheet })}><${Icon} n="plus" s=${13} w=${2.4} />${add}</button></span></div>`;
    ...
    return html`<div class="stack" style=${{ gap: '28px' }}>
      ...
      <div class="mw-split">
        <section id="money-accounts" class="stack-s" style=${{ gap: '12px' }}><${Head} title="Accounts" total=${fmt(D.cash + D.invest + D.property)} sub=${D.accts.length + (D.accts.length === 1 ? ' account' : ' accounts') + (groups.length > 1 ? ' · ' + groups.length + ' types' : '')} add="Add" sheet="addAccount" />
  ...
  // Imported statements that still move this balance: offer to mark them as already paid
  const ImportedNote = ({ where }) => {
    const { data, openSheet } = useApp();
    const n = K.importedOn(data, where).length;
    if (!n) return null;
