...
(function () {
  ...
  K.Home = function Home() {
    ...
    if (wide) {
      ...
      const W = ({ title, action, onAction, children, cls, style }) => html`<div class=${'card stack ' + (cls || '')} style=${Object.assign({ gap: '14px' }, style)}><div class="between"><h3 style=${{ fontSize: '16px' }}>${title}</h3>${action && html`<button class="pill-link" onClick=${onAction}>${action}<${Icon} n="next" s=${13} w=${2.2} /></button>`}</div>${children}</div>`;
      ...
      const flowW = html`<${W} cls="span2" title="Income and spending" action="Statistics" onAction=${() => go({ r: 'stats', tab: 'overview' })}>${S.length > 1 ? html`<${K.BarChart} groups=${S.map((x) => ({ label: x.m, values: [x.income, x.spending] }))} colors=${['var(--c2)', 'var(--c1)']} hi=${S.length - 1} fmtY=${fmt.k} h=${190} /><div class="legend"><span><i class="dotk" style=${{ background: 'var(--c2)' }}></i>Income</span><span><i class="dotk" style=${{ background: 'var(--c1)' }}></i>Spending</span><span>Last ${S.length} months</span></div>` : html`<${Empty} text="The chart fills in after your first month of activity. Importing a statement brings in history right away." action="Import a statement" onAction=${() => openSheet({ k: 'statement' })} />`}</${W}>`;
      const goalsW = html`<${W} title="Goals" action="All goals" onAction=${() => go({ r: 'plan', tab: 'goals' })}>${D.goals.length ? html`<div class="stack-s" style=${{ gap: '14px' }}>${D.goals.slice(0, 4).map((g) => html`<button key=${g.id} class="stack-s" style=${{ gap: '6px', width: '100%', textAlign: 'left' }} onClick=${() => go({ r: 'goal', id: g.id })}><span class="between small"><span style=${{ fontWeight: 600 }}>${g.name}</span><span class="num muted">${g.pct}%</span></span><${K.Segs} pct=${g.pct} n=${16} h=${6} color=${g.pct >= 100 ? 'var(--pos2)' : 'var(--acc)'} /><span class="tiny muted num">${fmt(g.savedNow)} of ${fmt(g.target)}</span></button>`)}</div>` : html`<${Empty} text="Save toward something: a trip, a cushion, a new laptop." action="Add goal" onAction=${() => openSheet({ k: 'addGoal' })} />`}</${W}>`;
      const recentW = html`<div class="stack-s"><${SectionHeader} title="Recent activity" action="See all" onAction=${() => go({ r: 'money', tab: 'activity' })} />${D.tx.length ? html`<div class="card tight list">${D.tx.slice(0, 6).map((t) => html`<${K.TxnRow} key=${t.id} t=${t} />`)}</div>` : html`<div class="card"><${Empty} text="Expenses, income and transfers appear here." action="Add expense" onAction=${() => openSheet({ k: 'expense' })} /></div>`}</div>`;
      const insW = html`<div class="stack-s"><${SectionHeader} title="Worth a look" action="Insights" onAction=${() => go({ r: 'stats', tab: 'insights' })} />${insights.length ? html`<div class="stack-s" style=${{ gap: '12px' }}>${insights.slice(0, 3).map((i) => html`<${InsightCard} key=${i.id} i=${i} compact=${true} />`)}</div>` : html`<div class="card"><${Empty} text="Nothing needs your attention right now." /></div>`}</div>`;
      const upcomingW = html`<div class="stack-s"><${SectionHeader} title="Coming up" action="Bills" onAction=${() => go({ r: 'plan', tab: 'bills' })} />${D.upcoming.length ? html`<div class="card tight list">${D.upcoming.slice(0, 7).map((u, i) => html`<${UpcomingItem} key=${i} u=${u} />`)}</div>` : html`<div class="card"><${EmptyState} icon="calendar" title="Nothing scheduled" text="Add bills, subscriptions, loans and income to see what’s coming." action="Add a bill" onAction=${() => openSheet({ k: 'addBill' })} /></div>`}</div>`;
      return html`<div class="stack" style=${{ gap: '26px' }}>
        ...
        <div class="between" style=${{ flexWrap: 'wrap', gap: '12px' }}><${QuickRow} /><${ContextFilter} show=${['scope', 'currency']} /></div>
        <${Setup} />
        <div class="grid w3" style=${{ alignItems: 'start' }}>
          <div class="stack span2"><${SafeHero} />${(featured || credit) && html`<div class=${featured && credit ? 'grid w2' : 'stack'} style=${{ alignItems: 'stretch' }}>${featured}${credit}</div>`}</div>
          <div class="stack">${upcomingW}${tripCard}</div></div>
        <div class="grid w3" style=${{ alignItems: 'start' }}>${spendW}${budgetW}${nwW}</div>
        <div class="grid w3" style=${{ alignItems: 'start' }}>${flowW}${goalsW}</div>
        <div class="grid w2" style=${{ alignItems: 'start' }}>${recentW}${insW}</div></div>`;
    }
